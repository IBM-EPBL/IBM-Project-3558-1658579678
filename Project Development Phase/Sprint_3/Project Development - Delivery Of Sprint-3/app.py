...
def Dashboard():
    ...
    if session['loggedin'] == True:
        ...
        exp_limit = record['BAL_LIMIT']
        expint = record['EXPENSE']
        ...
        if request.method == 'POST':
            ...
            if expint >= exp_limit:
                message = Mail(
                    from_email='',# your mail id i.e senders mail id
                    to_emails=session['id'],
                    subject='Expense Alert Mail',
                    html_content='<p>You have exceeded your expense limit.</p><br><p>Your Available Balance is <strong>%s</strong></p>'%bal_limit
                )
                try:
